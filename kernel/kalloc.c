...
kfree(void *pa)
{
  ...
  acquire(&ref_lock);
  if(page_ref[PAGE_INDEX(pa)] > 1) {
    page_ref[PAGE_INDEX(pa)]--;
    release(&ref_lock);
    return;
  }
  page_ref[PAGE_INDEX(pa)] = 0;
  release(&ref_lock);
...
alloc_cow(pagetable_t pagetable, uint64 va) {
  ...
  pte_t *pte = walk(pagetable, va, 0); // 通过页表获取虚拟地址对应的页表项
  if(pte == 0) return -1;
  ...
  if(flags & PTE_COW) {
    ...
    uvmunmap(pagetable, va, 1, 1);// 取消之前的映射
